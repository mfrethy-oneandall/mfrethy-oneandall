...
- Active city nodes:
  ...
  - **sanctuary** (Dedicated, Ubuntu 24.04.4, i9/32GB, `100.100.202.16`): always-on backup. Local model: `qwen3:14b` via Ollama.
- Samuel MCP endpoint (primary): `http://127.0.0.1:5100/mcp` (Forge-local; will move to `100.112.13.70:5100` when Capital returns)
- Dev Hive: **retired on Forge** (`com.samuel.local.devhive` disabled). Sanctuary backup coordinator at `http://100.100.202.16:5120`.
...
--- LOCAL INFERENCE ---
- All inference on Forge runs through **LM Studio at `http://127.0.0.1:1234`**. Ollama is uninstalled from Forge.
- Primary model: `qwen3.5-35b-a3b` (GGUF MoE, 64k ctx)
- Embeddings: `text-embedding-mxbai-embed-large-v1` (GGUF, 1024d)
- Sanctuary: `qwen3:14b` via Ollama at `http://100.100.202.16:11434`

--- NON-NEGOTIABLE OPERATING RULES ---
1. **Samuel MCP first**: Use samuel MCP tools for HA inspection, config reads, entity state, health checks.
2. **Local model first**: Prefer LM Studio (Forge) or Ollama (Sanctuary). Escalate to cloud only for extreme complexity.
3. **No Dev Hive on Forge**: Do not use devhive_enqueue, hive_research, or hive_status — DevHive is retired on Forge.
